...
function renderCodeHandoffPage(params: {
  code: string;
  state: string;
  ...
  const statusLine =
    params.reason === "expired"
      ? "State parameter expired on the callback server."
      : "State parameter was not found on the callback server.";

  return `
<!DOCTYPE html>
<html>
<head><title>Complete Authentication</title></head>
<body style="font-family: system-ui; max-width: 800px; margin: 40px auto; padding: 20px;">
  <h1 style="color: #d35400;">Complete Authentication Manually</h1>
  <p>${statusLine}</p>
  <p>This happens when the OAuth link was generated by a different process or after a restart. The authorization code below is still often valid for a short time.</p>
  <h2>Next step in Claude</h2>
  <pre style="background: #f8f9fa; padding: 12px; border-radius: 6px; white-space: pre-wrap;">{
  ...
  <p style="margin-top: 18px;">
    ${MCP_SERVER_URL ? `If needed, restart MCP auth flow from: <code>${MCP_SERVER_URL}</code>` : ""}
  </p>
</body>
</html>
`;
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (url.pathname === "/callback") {
      ...
      if (!stateData) {
        return new Response(renderCodeHandoffPage({ code, state, reason: "invalid" }), {
      ...
      if (Date.now() - stateData.createdAt > 10 * 60 * 1000) {
        ...
        return new Response(renderCodeHandoffPage({ code, state, reason: "expired" }), {
          headers: { "Content-Type": "text/html" },
          status: 200,
        });
      }
